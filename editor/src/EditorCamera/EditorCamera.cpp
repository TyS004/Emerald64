...
Editor::EditorCamera::EditorCamera(){
    ...
    this->transform = new E64::ECS::Transform{};
...
void Editor::EditorCamera::OnUpdate(){
    glm::vec3 direction;
    ...
    if(E64::Input::isKeyDown(SDL_SCANCODE_W)) { transform->position += 0.05f * camera_front; }
    if(E64::Input::isKeyDown(SDL_SCANCODE_S)) { transform->position -= 0.05f * camera_front; }
    if(E64::Input::isKeyDown(SDL_SCANCODE_A)) { transform->position -= 0.05f * glm::normalize(glm::cross(camera_front, camera_up)); }
    if(E64::Input::isKeyDown(SDL_SCANCODE_D)) { transform->position += 0.05f * glm::normalize(glm::cross(camera_front, camera_up)); }
...
E64::ECS::Transform* Editor::EditorCamera::getTransform(){
    return this->transform;
}

void Editor::EditorCamera::setTransform(E64::ECS::Transform* transform){
